...
from .als_io import TemplateInfo, load_template, save, strip_to_tracks
...
from .spec import Clip, LoopGeometry
...
def _read_geometry(clip_xml: ET.Element, length: float) -> LoopGeometry:
    """Extract the source clip's loop/marker geometry verbatim."""
    loop = clip_xml.find("Loop")

    def f(parent: ET.Element, tag: str, default: float) -> float:
        el = parent.find(tag)
        return float(el.get("Value")) if el is not None else default

    loop_on_el = loop.find("LoopOn")
    return LoopGeometry(
        loop_start=f(loop, "LoopStart", 0.0),
        loop_end=f(loop, "LoopEnd", length),
        start_relative=f(loop, "StartRelative", 0.0),
        loop_on=(loop_on_el.get("Value") == "true") if loop_on_el is not None else True,
        out_marker=f(loop, "OutMarker", length),
        hidden_loop_start=f(loop, "HiddenLoopStart", 0.0),
        hidden_loop_end=f(loop, "HiddenLoopEnd", length),
        current_start=f(clip_xml, "CurrentStart", 0.0),
        current_end=f(clip_xml, "CurrentEnd", length),
    )


def read_midi_clip(
    clip_xml: ET.Element,
) -> tuple[str, float, list[NoteEvent], LoopGeometry]:
    """Parse a <MidiClip> element into (name, length_beats, events, geometry)."""
    ...
    return name, length, events, _read_geometry(clip_xml, length)
...
def render_midi_to_dmx(
    ...
    *,
    ...
    If `only_tracks` is set, the saved set is stripped down to just the
    `notes_track` and `plugin_track` (all other tracks removed) — handy for a
    clean drag-source set. Don't combine with overwriting the input in place."""
    src = Path(input_path)
    ...
    for slot_idx, clip_el in source_clips:
        name, length, events, geometry = read_midi_clip(clip_el)
        if not events:
            continue
    ...
    if only_tracks:
        if out == src:
            raise ValueError("only_tracks would strip the input; pass a separate output_path")
        strip_to_tracks(template.root, {notes_track, plugin_track})
        # track indices shifted; re-find the plugin track for validation
        tracks = list(template.root.find("LiveSet/Tracks"))
        plugin_track_idx = next(
            i for i, tr in enumerate(tracks)
            if tr.find("Name/EffectiveName") is not None
            and tr.find("Name/EffectiveName").get("Value") == plugin_track
        )
